...
// Let's Learn Mathematics (category 32): one K-12 ladder. Distinct from the Math subject
// inside Grades 1 to 12, which is a grade's worth of practice rather than the road in order.
// Let's Learn Chemistry (category 33): one K-12 ladder. Distinct from the Chemistry subject
...
const CHEMCOURSE_SUBJECTS = [
  ...
  { key: "materials", label: "3 · Materials",          emoji: "🧱" },
  { key: "changes",   label: "4 · Changes",            emoji: "🔥" },
  { key: "mixtures",  label: "5 · Mixtures",           emoji: "🧴" },
  { key: "atoms",     label: "6 · Atoms & Elements",   emoji: "⚛️" },
  { key: "table",     label: "7 · Periodic Table",     emoji: "🧪" },
  { key: "molecules", label: "8 · Molecules",          emoji: "🔗" },
  { key: "reactions", label: "9 · Reactions",          emoji: "💥" },
  { key: "acids",     label: "10 · Acids & Bases",     emoji: "🧫" },
  { key: "mole",      label: "11 · The Mole",          emoji: "📊" },
  { key: "everyday",  label: "12 · Every Day",         emoji: "🍞" }
];

const VACOURSE_SUBJECTS = [
  { key: "whatis",      label: "1 · What Is Visual Art?", emoji: "🎨" },
  { key: "line",        label: "2 · Line",                emoji: "✏️" },
  { key: "shapeform",   label: "3 · Shape & Form",        emoji: "🔷" },
  { key: "colour",      label: "4 · Colour",              emoji: "🌈" },
  { key: "texture",     label: "5 · Texture & Pattern",   emoji: "🧶" },
  { key: "value",       label: "6 · Light & Shadow",      emoji: "🌗" },
  { key: "space",       label: "7 · Perspective",         emoji: "🛣️" },
  { key: "composition", label: "8 · Composition",         emoji: "🖼️" },
  { key: "figure",      label: "9 · Drawing People",      emoji: "🧍" },
...
const PHYSCOURSE_SUBJECTS = [
  { key: "whatis",      label: "1 · What Is Physics?",   emoji: "🔭" },
  { key: "forces",      label: "2 · Push & Pull",        emoji: "💪" },
  { key: "motion",      label: "3 · Motion & Speed",     emoji: "🏃" },
  { key: "gravity",     label: "4 · Gravity",            emoji: "🪐" },
  { key: "friction",    label: "5 · Friction",           emoji: "🧊" },
  { key: "machines",    label: "6 · Simple Machines",    emoji: "⚙️" },
  { key: "energy",      label: "7 · Energy",             emoji: "⚡" },
  { key: "heat",        label: "8 · Heat & Temperature", emoji: "🌡️" },
  { key: "light",       label: "9 · Light",              emoji: "💡" },
...
const MATHCOURSE_SUBJECTS = [
  ...
  { key: "counting",   label: "2 · Numbers",          emoji: "🔢" },
  { key: "addsub",     label: "3 · Add & Subtract",   emoji: "➕" },
  { key: "times",      label: "4 · Times & Divide",   emoji: "✖️" },
  { key: "fractions",  label: "5 · Fractions",        emoji: "🍕" },
  { key: "decimals",   label: "6 · Decimals & %",     emoji: "💷" },
  { key: "shape",      label: "7 · Shape & Space",    emoji: "📐" },
  { key: "measure",    label: "8 · Measurement",      emoji: "📏" },
  { key: "ratio",      label: "9 · Ratio & Rates",    emoji: "⚖️" },
...
// STEM Activities for Kids (category 38): twenty hands-on Science, Technology, Engineering and
// Math activities for grade schoolers to do with a parent, plus the scientific-method lesson
// that sits underneath all of them. Same two-subject shape as Paper (30): one browsable list,
// one lesson explaining the idea behind it.
// Health & My Body (category 39): how a body works and how to look after it. Every figure in
// js/health.js comes from a named authority (CDC, USDA MyPlate, AAP, ADA) and is cited in the
// lesson text, because health guidance a parent cannot trace is worse than none.
const HEALTH_SUBJECTS = [
  { key: "body",  label: "Your Body",     emoji: "🫀" },
  { key: "eat",   label: "Food Is Fuel",  emoji: "🍎" },
  { key: "germs", label: "Germs & Hands", emoji: "🧼" },
  { key: "teeth", label: "Happy Teeth",   emoji: "🦷" },
  { key: "sleep", label: "Sleep",         emoji: "😴" },
  { key: "move",  label: "Move",          emoji: "🤸" },
  { key: "safe",  label: "Staying Safe",  emoji: "🪖" }
];
const STEM_SUBJECTS = [
  { key: "activities", label: "STEM Activities",     emoji: "🧪" },
  { key: "method",     label: "Think Like a Scientist", emoji: "🔬" }
];
function subjectsFor(g) {
  ...
  if (g === 38) return STEM_SUBJECTS;
  if (g === 39) return HEALTH_SUBJECTS;
...
function gradeName(g) {
  ...
  if (g === 38) return "STEM Activities for Kids";
  if (g === 39) return "Health & My Body";
  return "Grade " + g;
}
...
const STRIPE_PAYMENT_LINK = "https://buy.stripe.com/7sY6oH1eWep6cMs5yQ3gk06";
...
// ============================================================
// Fresh problems for a Let's Learn Mathematics unit, mixed with its concept questions so a
...
function genPhysCourse(lesson) {
  const gen = (typeof PHYS_GEN !== "undefined") && PHYS_GEN[lesson.physGen];
  const bank = shuffleArr((lesson.questions || []).slice());
  if (!gen) return bank.slice(0, 6);
  const made = [], seen = {};
  for (let i = 0; i < 80 && made.length < 4; i++) {
...
function genVACourse(lesson) {
  const gen = (typeof VA_GEN !== "undefined") && VA_GEN[lesson.vaGen];
  const bank = shuffleArr((lesson.questions || []).slice());
  if (!gen) return bank.slice(0, 6);
  const made = [], seen = {};
  for (let i = 0; i < 80 && made.length < 4; i++) {
...
function genMathCourse(lesson) {
  const gen = (typeof MATH_GEN !== "undefined") && MATH_GEN[lesson.mathGen];
  const concept = shuffleArr((lesson.questions || []).slice()).slice(0, 2);
  if (!gen) return concept.concat(shuffleArr((lesson.questions || []).slice()).slice(0, 4));
  const made = [], seen = {};
  for (let i = 0; i < 60 && made.length < 4; i++) {
    const q = gen();
    if (q && !seen[q.q]) { seen[q.q] = 1; made.push(q); }
  }
  return shuffleArr(made.concat(concept));
}
...
function lessonView() {
  ...
  let body = "";
  // A diagram may be a string or a function returning one. The maths course uses functions so
  // its shared drawing kit is looked up when the picture is drawn, not when the file loads.
  if (lesson.diagram) {
  ...
  if (lesson.project) {
    ...
    const track = (t, icon) => !t ? "" : `
      <div class="vp-track">
        <h5>${icon} ${esc(t.band)}</h5>
        <ol>${t.steps.map(x => `<li>${esc(x)}</li>`).join("")}</ol>
      </div>`;
    body += `<div class="vproj">
      <h3>🛠️ Project: ${esc(P.name)}${P.mins ? ` <i class="vp-mins">⏱ about ${P.mins} min</i>` : ""}</h3>
      <div class="vp-needs"><h5>🧺 What you need</h5><ul>${P.materials.map(m => `<li>${esc(m)}</li>`).join("")}</ul></div>
      ${P.safety ? `<p class="vp-safe">⚠️ <b>Stay safe:</b> ${esc(P.safety)}</p>` : ""}
      <div class="vp-cols">${track(P.solo, "🧑‍🎨")}${track(P.together, "👨‍👧")}</div>
    </div>`;
  }
  if (lesson.resources) {
  ...
  return `<div class="view">
    ...
    <div class="card" id="lesson-card">
      ...
      ${lesson.mathFormulas ? `<div class="fcard"><h3>📐 Formulas to know</h3><table class="ftable"><tr><th>For…</th><th>The formula</th><th>What it means</th></tr>${lesson.mathFormulas.map(f => `<tr><td>${esc(f.name)}</td><td class="fmath">${esc(f.f)}</td><td class="fmean">${esc(f.means)}</td></tr>`).join("")}</table></div>` : ""}
      ${body}
    ...
    <div class="plans">
      <div class="plan">
        ...
        <ul>
          ...
          <li class="no">The 18 "Let's Learn" subject categories</li>
      ...
      <div class="plan best">
        ...
        <ul>
          ...
          <li><b>23 "Let's Learn" categories</b>: Geography, Space, Biology, Chemistry, Physics, Mathematics, Essay Writing, Visual Arts, Music, Computer Science, Spanish, Geology, Paleontology, Weather &amp; Oceans, Time &amp; Money, The History of Us, Feelings &amp; Kindness, Kids &amp; Family Jokes, Paper Activities, STEM Activities for Kids, Health &amp; My Body, Coloring &amp; Maze Worksheets &amp; Books</li>
...
const App = {
  ...
  openGrade(g) {
    ...
    const dflt = g === 0 ? "alphabet" : g === 13 ? "globe" : g === 14 ? "coloring"
               ...
               : g === 21 ? "greetings" : g === 23 ? "clock" : g === 24 ? "spacecourse" : g === 25 ? "feelings" : g === 26 ? "digsite" : g === 28 ? "weather" : g === 29 ? "jokeshow" : g === 30 ? "paperstudio" : g === 31 ? "whatis" : g === 32 ? "whatis" : g === 33 ? "whatis" : g === 34 ? "whatis" : g === 35 ? "whatis" : g === 36 ? "whatis" : g === 37 ? "whatis" : g === 38 ? "activities" : g === 39 ? "body" : "math";
