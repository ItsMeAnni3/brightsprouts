...
| ⭐ Monetization | Free plan (Grades 1–2, 10 stories, 2 custom stories) vs Premium **$6.99/month + state sales tax** (everything included). Locked content shows 🔒 and funnels users to sign up or upgrade |
...
3. **Premium is priced at $6.99/month plus tax**, and includes absolutely everything: every grade, all 15 "Let's Learn" categories, the Code Terminal, the arcade, the Globe and Sprout, for every child in the family.

   `PRICE` near the top of `js/app.js` controls what the page SAYS. What a customer is actually CHARGED lives in Stripe, on the product behind `STRIPE_PAYMENT_LINK`. **Change both together**, or the site advertises one price and the card is billed another. `RULES` controls what is free.

### Turning on REAL payments (the demo checkout → Stripe)

The upgrade button currently simulates a purchase. To collect real money:

1. Create a free account at **stripe.com** → Products → add "Premium Family, $6.99/month subscription".
